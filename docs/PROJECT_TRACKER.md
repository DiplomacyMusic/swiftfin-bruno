...
| [~] | Ebert shelves: star caption + score order + custom grid + genre pills | `BrunoEbertContentView`, `BrunoEbert`, `BrunoEbertView`, `BrunoShelfRow` (`showsEbertStars`), `BrunoBoxSetShelvesView`, `BrunoCategoryCardRow` + `enrich/data/ebert_tags_by_tmdb.json` + `Apply-Enrich-Tags.command` | PR #57 (builds; tags STAMPED 2026-06-29). ★★★½ line-2 caption (INV-1 clone); inline + grid ordered by stars (Up high-first / Down low-first, `BrunoEbert`); "Show all" redirected off stock `ItemLibrary` to `BrunoEbertView` (clone of Rewatchables) with the Movies-tab "Browse by" genre pills. **Owner action:** merge PR #57 + rebuild; pill focus traversal is the on-device check. Hero uses `Curated02` (a wider `EbertHero` asset is a follow-up). |
