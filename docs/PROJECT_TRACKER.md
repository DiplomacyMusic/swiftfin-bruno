...
| [~] | Oscars: reverse-chron order + *Winner/Nominee (Year)* caption | `BrunoOscarContentView`, `BrunoOscar`, `BrunoBoxSetShelvesView`, `BrunoBoxSetGridView`, `BrunoCategoryCardRow` + `enrich/p9_oscars.py` | Code on branch `claude/modest-goodall-5fe0fc` (builds). **Owner action:** run `LIVE=1 python3 enrich/p9_oscars.py` to stamp `oscar:` tags, then rebuild — captions are blank until then. |
