...
| [~] | Ebert: ★★★½ star-rating caption on Ebert shelves | `BrunoEbertContentView`, `BrunoShelfRow` (`showsEbertStars`), `BrunoCategoryShelves` + `enrich/data/ebert_tags_by_tmdb.json` + `Apply-Enrich-Tags.command` | App code added (builds): line-2 stars from the `ebert-stars:<n>` tag, threaded per curated child by name prefix "ebert" (lights up "Ebert Thumbs Up"/"Down"). INV-1 geometry-faithful clone of the Oscar caption. **Owner action:** same `apply` run stamps the `ebert-stars` tags (682 films). |
