...
| **Oscar category collections** (Best Picture/Directing/Acting/Cinematography/Score/Screenplay) | **BoxSet** under **Curated** (now consolidated into one synthetic "Oscars" tile app-side, #40) | Curated drill-in shelves + Home `curated` generator | Script (Phase 4 `oscars.json` join) + the app-side Oscars consolidation (`BrunoCategoryCardRow.swift:72`). |
